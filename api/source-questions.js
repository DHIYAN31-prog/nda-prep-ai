...
export default async function(req,res){
 ...
 const qs=[]; for(let i=0;i<pool.length&&qs.length<count;i++){const s=pool[i];const others=pool.filter((x,n)=>x!==s&&n!==i);const opts=[s,others[(i*3)%others.length]||"Not supported by the uploaded source.",others[(i*5+1)%others.length]||"Not supported by the uploaded source.",others[(i*7+2)%others.length]||"Not supported by the uploaded source."];const order=[0,1,2,3].sort((a,b)=>key(i,a)-key(i,b));const out=order.map(n=>opts[n].slice(0,260));qs.push({question:["According to the uploaded PDF, which statement is supported by the source?","Which statement matches the uploaded study material?","Select the statement directly supported by the uploaded PDF."][i%3],options:out,answer:["A","B","C","D"][out.indexOf(s.slice(0,260))],explanation:"The correct option is reproduced from the selected uploaded PDF text. Review the source sentence for context.",topic:"Uploaded PDF • 1 Mark",difficulty:"Source-based",marks:1,source:"User-uploaded PDF"});}
